 ...
 * Returns paths relative to .cursor/ (e.g. "rules/foo.mdc").
 */
async function getRemoteDiffPaths(cacheDir) {
  ...
  try {
    const { stdout } = await execAsync(`git diff --name-only HEAD ${remoteRef} -- .cursor/`, {
      cwd: cacheDir
    });
    const lines = stdout.trim().split('\n').filter(Boolean);
    return lines.map(p => p.replace(/^\.cursor[/\\]/, '').replace(/\\/g, '/'));
  } catch {
    return [];
  }
}
